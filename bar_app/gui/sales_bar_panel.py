...
class SalesBarPanel(ttk.Frame):
    def __init__(self, parent, db_conn, on_sale_confirmed):
        ...
        main_frame.columnconfigure(0, weight=1) # Product List
        main_frame.columnconfigure(1, weight=2) # Current Order
        main_frame.columnconfigure(2, weight=1) # Finalize & History
        main_frame.rowconfigure(0, weight=1)

        # --- Column 1: Product List ---
        product_list_frame = ttk.LabelFrame(main_frame, text="Available Products")
        product_list_frame.grid(row=0, column=0, sticky="nsew", padx=(0, 5))
        product_list_frame.rowconfigure(0, weight=1)
        product_list_frame.columnconfigure(0, weight=1)

        self.product_tree = ttk.Treeview(product_list_frame, columns=("Name", "Shortcut", "Price"), show="headings")
        self.product_tree.heading("Name", text="Product")
        self.product_tree.heading("Shortcut", text="Shortcut")
        self.product_tree.heading("Price", text="Price (COP)")
        self.product_tree.column("Name", width=120)
        self.product_tree.column("Shortcut", width=60)
        ...
        selection_frame = ttk.LabelFrame(center_frame, text="Add Product")
        ...
        current_order_frame = ttk.LabelFrame(center_frame, text="Current Order")
    ...
    def load_product_list(self):
        ...
        for prod in inventory.get_products(self.db_conn):
            # prod = (id, name, quantity, price, shortcut)
            self.product_tree.insert("", "end", values=(prod[1], prod[4], f"{prod[3]:,.0f}"))
    ...
    def add_product_to_order(self, event=None):
        ...
        self.quantity_entry.delete(0, "end"); self.quantity_entry.insert(0, "1")
    ...
    def process_payment(self):
        if not self.current_order_items:
            messagebox.showerror("Error", "The order is empty."); return
        ...
        try:
            ...
            if paid_with < total_order: raise ValueError
        ...
        self.clear_order()
        self.update_history()
        self.load_product_list() # Refresh product list for stock changes
